...
def ads():
 expire()
 with conn() as c:r=c.execute("SELECT business,player,category,location,text,expires_at FROM ads WHERE status='active' AND expires_at>?",(int(time.time()),)).fetchall()
...
def create():
 ...
 return jsonify(adId=aid,status='pending_payment',price=pl['price'],durationMinutes=pl['minutes'],paymentAccountId=PAY_ACCOUNT or 'NOT_CONFIGURED',paymentReference=ref),201
@app.get('/api/ads/<aid>/status')
def status(aid):
 with conn() as c:r=c.execute('SELECT * FROM ads WHERE id=?',(aid,)).fetchone()
...
if __name__=='__main__':init();app.run(host='0.0.0.0',port=int(os.getenv('PORT','8081')),debug=False)
